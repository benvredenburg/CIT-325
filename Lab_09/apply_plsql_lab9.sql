...
-- Verify the contents of the AVENGER table.
SELECT * FROM avenger;

-- Create the FILE_LIST table.
...
-- Verify the contents of the FILE_LIST table.
DESC file_list
...
-- Verify signature of load_clob_from_file.
DESC load_clob_from_file

-- Create anonymous PL/SQL block to verify load_clob_from_file works correctly.
DECLARE
...
-- Update text_file_name column for each item_title in the ITEM table.
UPDATE item i
...
-- Verify that the text_file_name column has been updated for each item_title in the ITEM table.
COL text_file_name  FORMAT A16
COL item_title      FORMAT A42
SELECT   DISTINCT
         text_file_name
...
-- Create an update_item_description procedure
...
-- Execute update_item_description procedure.
EXECUTE update_item_description('Harry Potter');

-- Verify success of upload.
